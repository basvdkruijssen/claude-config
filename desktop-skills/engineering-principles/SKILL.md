...
description: Reference index of 23 engineering principles (core design, architecture, verification, delegation, meta) ported from pstack. Ask to apply one by name, e.g. "apply the fix-root-causes principle here".
---

# Engineering principles

23 short rules, grouped. Ported from pstack (cursor/plugins), MIT licensed, see NOTICE.md in the source repo. Each was originally its own skill file in Claude Code; bundled into one document here because Claude Desktop skills are uploaded as individual zips and 23 near-empty zips isn't practical. Ask for one by name.
