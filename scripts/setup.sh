...
# including ~/.claude/CLAUDE.md and the status-line script, so the script
...
echo "  installing claude-code-setup and claude-md-management (Anthropic's official helpers)..."
...
if [ -z "${SKIP_CHEZMOI_APPLY:-}" ]; then
  ...
  echo "  dotfiles applied (incl. ~/.claude/CLAUDE.md and ~/.claude/statusline-context.sh)."
fi
...
echo ""
echo "To update this machine later: 'chezmoi update' refreshes everything deployed to ~"
echo "(prompt, profile, ~/.claude/CLAUDE.md, status line); 'claude plugin update' refreshes"
echo "the plugins. Re-running this script does both and is safe to repeat."
